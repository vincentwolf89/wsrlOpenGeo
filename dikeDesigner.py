...
designFields = ["dijkvak","onderdeel","start_offset","end_offset","start_z","end_z"]
            ...
            with arcpy.da.SearchCursor(tempSelect, ("Shape@")) as sCur:
                for line in sCur:
                    ...
                    if start_offset >= 0: # river section
                        offsetLineStart = CopyParallelR(line[0], start_offset)
                        dfCur.insertRow((offsetLineStart, dijkvak,onderdeel,start_offset,start_z))

                    if start_offset < 0: # dike section
                        offsetLineStart = CopyParallelL(line[0], abs(start_offset))
                        dfCur.insertRow((offsetLineStart, dijkvak,onderdeel,start_offset,start_z))
    
                    if end_offset >= 0:
                        offsetLineEnd = CopyParallelR(line[0], end_offset)
                        dfCur.insertRow((offsetLineEnd, dijkvak,onderdeel,end_offset,end_z))
                    if end_offset < 0:
                        offsetLineEnd = CopyParallelL(line[0], abs(end_offset))
                        dfCur.insertRow((offsetLineEnd, dijkvak,onderdeel,end_offset,end_z))
