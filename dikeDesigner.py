...
## cut bik line into dike segments, mightn not be needed every run
# transects on endpoints of dike segments
# intersecting points with bik
# split line 

# iterate over design, groupby 
with arcpy.da.UpdateCursor(inputTable, designFields) as cursor:
    for k, g in groupby(cursor, lambda x: x[0]):
        ...
        for row in g:
            ...
            dikeSegmentSelect = arcpy.management.SelectLayerByAttribute(tempLayer, "NEW_SELECTION", "dijkvak = '{}'".format(dijkvak))
            tempSelect = arcpy.management.CopyFeatures(tempLayer, "tempSelect")

            ## bik section
            # transects on endpoints
            arcpy.management.GenerateTransectsAlongLines(
                in_features=tempSelect,
            ...
            # arcpy.analysis.Intersect(
            #     in_features="safe_binnenkruinlijn_rd #;tempTransects #",
            #     out_feature_class="tester",
            #     join_attributes="ALL",

            #     output_type="POINT"
            # )
            # split bik line with isects
            ...
            sectionLength = ([f[0] for f in arcpy.da.SearchCursor(tempSelect, 'SHAPE@LENGTH')][0])
            arcpy.management.GenerateTransectsAlongLines(
                in_features=tempSelect,
            ...
            # check if bik is needed as ref
            if ref =="bik":
                tempSelect = bikSegmentSelect
            

            # copy parallel
            with arcpy.da.SearchCursor(tempSelect, ("Shape@")) as sCur:
                for line in sCur:
